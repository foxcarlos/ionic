...
export class HistorialProvider {
  ...
  private crear_contacto( texto: string ){
      ...
      // Si se ejecuta desde el computador no se crea
      if( !this.platform.is('cordova')){
          console.error('Estamos en modo web');
          return;
      }
      ...
      contact.save().then(
          ()=> this.crear_toast('Contacto creado:' + nombre),
  ...
  private crear_email( texto: string ) {
      let [correo, asunto, cuerpo, ...resto] = texto.split(';');

      let correo2 = correo.slice(10);
      let asunto2 = asunto.slice(4);
      let cuerpo2 = cuerpo.slice(5);
      console.log(correo2, asunto2, cuerpo2);
  ...
  private crear_toast(msg: string){
